...
$registerAssets = function () use ($mapApiKey, $escUrl) {
    ...
    $prepareJs('js/jspdf.min.js', 'esc-jspdf', '1.5.3');
    ...
    $prepareCss('vendor/fontawesome/css/solid.min.css', 'esc-css-fontawesome-solid', '5.1.2');
    $prepareCss('css/app.css', 'esc-css-app', '1.0.2');
};


add_action('wp_head', function() use ($escUrl) {
    echo '<script>var escCalcDataUrl = "' . $escUrl . '/assets/js/esc_model.json?v=9";</script>';
});
...
add_shortcode(
    ...
    function ($options) use ($escUrl, $registerAssets) {
        ...
        wp_enqueue_script('esc-jspdf');
        wp_enqueue_script('esc-html2canvas');
        wp_enqueue_script('esc-calculator');
        wp_enqueue_script('esc-google-map');

        wp_enqueue_style('esc-css-bootstrap');
        wp_enqueue_style('esc-css-fontawesome-solid');
