...
logical : {
    ...
    RANK : function(val, range, order) {
        ...
        var sortedRange = range.sort(sort);

        var rank = sortedRange.map(
            function (rank) {
                return function (a, i, aa) {
                    return aa[i - 1] === a ? rank : rank = i + 1;
                };
            }(0)
        );

        return rank[sortedRange.indexOf(val)];
    },
...
sheet.fx.buildCellDependency = function(){
    ...
    if(typeof(formula[func]) == 'function'){
        //console.log(func + '('+ params.join(', ') +') => ' + formula[func].apply(this, params));
        return formula[func].apply(this, params);
    }

    for(category in formula){
        if(typeof(formula[category][func]) == 'function' ){
            //console.log(func + '('+ params.join(', ') +') => ' + formula[category][func].apply(this, params));
